        ...
        context('Negatives', function () {
            it('User should fail to create a nft with error collectionid', async function () {
                await expect(aiCooHub.connect(user).commitNewNFTIntoCollection({
                    ...
                    proof: [],
                })).to.be.revertedWithCustomError(aiCooHub, ERRORS.COLLECTIONID_NOT_EXIST);
            });
            it('UserTwo should fail to create a nft if collection owner not create 0 nft', async function () {
                await expect(aiCooHub.connect(userTwo).commitNewNFTIntoCollection({
                    ...
                    derivedModuleData: abiCoder.encode(['bool'], [false]),
                    proof: [],
                })).to.be.revertedWithCustomError(aiCooHub, ERRORS.JUST_OWNERCAN_PUBLISH_ROOT_NODE);
            });
            ...
            it('user/userTwo should fail to create a nft if derivedfrom error after owner create root node', async function () {
                ...
                await expect(aiCooHub.connect(userTwo).commitNewNFTIntoCollection({
                    ...
                    derivedModuleData: abiCoder.encode(['bool'], [false]),
                    proof: [],
                })).to.be.revertedWithCustomError(aiCooHub, ERRORS.DERIVEDFROM_NFT_NOT_EXIST);
                await expect(aiCooHub.connect(user).commitNewNFTIntoCollection({
                    collectionId: 0,
                    nftInfoURI: MOCK_URI,
                    derivedFrom: 2,
